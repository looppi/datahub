...
def test_orphan_datasets(
    ...
    pipeline = Pipeline.create(
        {
            ...
            "sink": {
                "type": "file",
                "config": {
                    "filename": f"{tmp_path}/powerbi_orphan_dataset_mces.json",
                },
            },
        }
    )
    ...
    pipeline.run()
    pipeline.raise_from_status()
    mce_out_file = "golden_test_orphan_datasets.json"

    mce_helpers.check_golden_file(
        pytestconfig,
        output_path=tmp_path / "powerbi_orphan_dataset_mces.json",
        golden_path=f"{test_resources_dir}/{mce_out_file}",
    )
